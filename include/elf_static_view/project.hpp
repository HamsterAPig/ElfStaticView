...
public:
    ...
    [[nodiscard]] ProjectModel scan(const std::string& file_path, const ScanOptions& options = {}) const;
    [[nodiscard]] ProjectModel dump(const std::string& file_path, const DumpOptions& options = {}) const;
...
public:
    ...
    [[nodiscard]] std::vector<StaticAddressResult> query(const StaticAddressQueryOptions& options = {});
...
[[nodiscard]] std::string render_export_document(const ExportDocument& document, const ExportOptions& options = {});
[[nodiscard]] ExportDocument parse_export_bytes(const std::string& bytes, ExportFormat format);
[[nodiscard]] ExportDocument parse_export_bytes_auto(const std::string& bytes);
[[nodiscard]] ProjectModel build_lightweight_project_model(const LightweightExport& document,
                                                           const std::string& source_path = {});
[[nodiscard]] ProjectSnapshot build_lightweight_project_snapshot(const LightweightExport& document,
                                                                 const std::string& source_path = {});
[[nodiscard]] ImportedProjectData import_project_data_bytes(const std::string& bytes,
                                                            const std::string& source_path = {});
[[nodiscard]] ExportDocument load_export_file(const std::string& file_path, ExportFormat format);
...
[[nodiscard]] ProjectSnapshot build_export_snapshot(const ProjectSnapshot& snapshot,
                                                    const SnapshotExportOptions& options = {});
[[nodiscard]] LightweightExport build_lightweight_export(const ProjectModel& model, const ExportOptions& options = {});
[[nodiscard]] LightweightExport build_lightweight_export(const std::vector<ExpandedNode>& nodes,
                                                         const ExportOptions& options = {});
[[nodiscard]] std::vector<StaticAddressResult> query_static_addresses(const ProjectModel& model,
                                                                      const StaticAddressQueryOptions& options = {});
[[nodiscard]] std::vector<StaticAddressResult> query_static_addresses_from_file(
    const std::string& elf_path, const StaticAddressQueryOptions& options = {}, const LoadPolicy& load_policy = {});
