...
$tempDir = Join-Path $env:TEMP ("elf-static-view-rnglists-startx-length-" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  if ($rnglists.Length -lt 0x25) {
    throw ".debug_rnglists 长度不足，无法改写成 startx_length"
  }

  # 直接重写 ranges payload：
  #   DW_RLE_startx_length 0x0, 0x17
