...
int main(int argc, char** argv)
{
    ...
    constexpr unsigned long flags = DW_DLC_TARGET_LITTLEENDIAN | DW_DLC_POINTER64 | DW_DLC_OFFSET32 |
                                    DW_DLC_ELF_OFFSET_SIZE_64 | DW_DLC_SYMBOLIC_RELOCATIONS;
    ...
    for (Dwarf_Unsigned section_index = 0; section_index < section_count; ++section_index) {
        ...
        const auto section_name = mapped_index < g_section_names.size() ? g_section_names[mapped_index] : std::string();
        if (section_name == ".debug_sup" && bytes != nullptr && length > 0) {
            debug_sup_bytes = std::vector<std::uint8_t>(reinterpret_cast<const std::uint8_t*>(bytes),
