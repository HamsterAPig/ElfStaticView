...
$tempDir = Join-Path $env:TEMP ("elf-static-view-gcc-ref-sup4-" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  # 把 DW_FORM_ref4(0x13) 改成 DW_FORM_ref_sup4(0x1c)，宽度保持 4 字节。
  $abbrev[$matchIndex + 12] = 0x1c
