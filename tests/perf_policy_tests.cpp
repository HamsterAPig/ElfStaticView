...
void verify_invalid_compile_unit_rules_fallback() {
  ...
  expect_true(state.load_policy.compile_unit_path_rules_text == expected,
              "非法编译单元规则应回退到默认规则");
...
void verify_cli_load_policy_uses_yaml_defaults() {
  ...
  expect_true(!policy.exclude_runtime_only_variables,
              "CLI 应读取 YAML 中的 exclude_runtime_only_variables");
