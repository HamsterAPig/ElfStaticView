...
- 使用 CLI 扫描或导出分析结果
- 将当前分析结果导出为 JSON / 私有二进制快照，并重新载入快照
- 在 GUI 导出时独立选择导出格式、数据来源、内容粒度和敏感信息策略
...
安装或 `add_subdirectory` 后链接 `elf_static_view::core`。第三方侧主要消费导出的数据文件：

```cpp
#include "elf_static_view/project.hpp"

auto document = elf_static_view::load_export_file(
    "snapshot.esv",
    elf_static_view::ExportFormat::BinaryPrivate);

if (document.payload_kind == elf_static_view::ExportPayloadKind::VariableSummary) {
