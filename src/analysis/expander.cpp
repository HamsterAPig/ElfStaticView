...
[[nodiscard]] bool has_expandable_children(const TypeNode& type) {
  return type.kind == TypeKind::Array || type.kind == TypeKind::Struct ||
         type.kind == TypeKind::Class || type.kind == TypeKind::Union;
}
...
next_remaining_depth(const std::optional<std::size_t> remaining_depth) {
...
Expander::Expander(const std::vector<TypeNode>& types,
                   const std::size_t expand_depth,
                   const bool lazy_expand_children)
  : expand_depth_(expand_depth),
    lazy_expand_children_(lazy_expand_children) {
