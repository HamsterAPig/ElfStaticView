...
namespace {
    ...
    template <typename T> void append_optional_number_value(std::ostringstream& stream, const std::optional<T>& value)
    {
        if (value.has_value()) {
            stream << value.value();
        } else {
    ...
    template <typename T> std::optional<T> parse_optional_number(const YAML::Node& node, const std::string& field_name)
    {
                             ...
                             const bool trailing_comma = true)
    {
        ...
        for (std::size_t index = 0; index < address.location_ranges.size(); ++index) {
            ...
            stream << ",\n";
            append_bool_field(stream, "debug_addr_unavailable", range.debug_addr_unavailable, level + 3, false);
            append_indent(stream, level + 2);
            stream << '}';
    ...
    void append_type_node(std::ostringstream& stream, const TypeNode& type, const int level)
    {
        ...
        append_array(
            stream, "members", type.members.size(), level + 1, [&](const std::size_t index, const int item_level) {
    ...
    void append_raw_dwarf_attribute(std::ostringstream& stream, const RawDwarfAttribute& attribute, const int level)
    {
        append_indent(stream, level);
        stream << "{\n";
    ...
    void append_raw_dwarf_die(std::ostringstream& stream, const RawDwarfDie& die, const int level)
    {
        ...
        append_array(
            stream, "attributes", die.attributes.size(), level + 1, [&](const std::size_t index, const int item_level) {
                              ...
                              const ExpandedNodePathMode path_mode = ExpandedNodePathMode::Internal)
    {
        ...
        append_array(stream,
                     "compile_units",
                     model.compile_units.size(),
                     level + 1,
                     [&](const std::size_t index, const int item_level) {
                         append_compile_unit_record(stream, model.compile_units[index], item_level);
                     });
        append_array(
            stream, "types", model.types.size(), level + 1, [&](const std::size_t index, const int item_level) {
                append_type_node(stream, model.types[index], item_level);
            });
        append_array(
            stream, "symbols", model.symbols.size(), level + 1, [&](const std::size_t index, const int item_level) {
                append_variable_record(stream, model.symbols[index], item_level);
            });
        append_array(
            stream, "expanded", model.expanded.size(), level + 1, [&](const std::size_t index, const int item_level) {
        ...
        append_number_field(
            stream, "variable_count_before_filter", model.metrics.variable_count_before_filter, level + 2);
        append_number_field(
            stream, "variable_count_after_filter", model.metrics.variable_count_after_filter, level + 2);
        append_number_field(
            stream, "skipped_compile_unit_count", model.metrics.skipped_compile_unit_count, level + 2, false);
        append_indent(stream, level + 1);
        stream << "}\n";
        append_indent(stream, level);
        stream << '}';
    ...
    AddressInfo parse_address_info(const YAML::Node& node)
    {
        ...
        if (const auto ranges = node["location_ranges"]; ranges && ranges.IsSequence()) {
            for (const auto& range_node : ranges) {
                ...
                range.debug_addr_unavailable = debug_addr_unavailable ? debug_addr_unavailable.as<bool>() : false;
    ...
    ProjectModel parse_project_model(const YAML::Node& node)
    {
        ...
        if (const auto metrics = node["metrics"]; metrics) {
            ...
            model.metrics.deduplicate_ms = parse_optional_number<std::uint64_t>(metrics, "deduplicate_ms").value_or(0);
...
ProjectSnapshot build_export_snapshot(const ProjectSnapshot& snapshot, const SnapshotExportOptions& options)
{
...
std::string render_snapshot_json(const ProjectSnapshot& snapshot, const SnapshotExportOptions& options)
{
...
std::string render_raw_dwarf_json(const RawDwarfDocument& document)
{
    ...
    append_array(
        stream, "compile_units", document.compile_units.size(), 1, [&](const std::size_t index, const int item_level) {
