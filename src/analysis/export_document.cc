...
namespace {
    ...
    [[nodiscard]] std::string render_binary_full_snapshot(const ProjectSnapshot& snapshot, const ExportOptions& options)
    {
                                ...
                                std::vector<LightweightVariableRecord>& output)
    {
        for (const auto& node : nodes) {
            ...
            const bool limit_array_children = node.type_kind == TypeKind::Array && max_array_elements > 0 &&
            ...
            for (std::size_t index = 0; index < child_count; ++index) {
                collect_expanded_nodes(std::vector<ExpandedNode>{node.children[index]}, max_array_elements, output);
            }
        }
    }

    [[nodiscard]] std::string make_unique_lightweight_path(const std::string& raw_path,
...
LightweightExport build_lightweight_export(const std::vector<ExpandedNode>& nodes, const ExportOptions& options)
{
    LightweightExport document;
    document.source = options.source;
    document.include_sensitive_info = options.include_sensitive_info;
...
ProjectModel build_lightweight_project_model(const LightweightExport& document, const std::string& source_path)
{
    ProjectModel model;
    ...
    for (const auto& variable : document.variables) {
        ...
        node.availability = variable.address.has_value() ? Availability::StaticAddressKnown : Availability::Unavailable;
...
ProjectSnapshot build_lightweight_project_snapshot(const LightweightExport& document, const std::string& source_path)
{
...
ImportedProjectData import_project_data_bytes(const std::string& bytes, const std::string& source_path)
{
