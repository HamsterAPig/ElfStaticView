...
namespace {
    ...
    struct QuerySignature {
        ...
        {
            return name_query_text == options.name_query_text && path_rules_text == options.path_rules_text &&
    ...
    [[nodiscard]] bool matches_path_rules(const std::vector<PathRule>& rules, const std::string& lowered_path)
    {
        if (rules.empty()) {
            return true;
        }
    ...
    [[nodiscard]] bool matches_name_tokens(const std::vector<std::string>& tokens, const PreparedNodeText& prepared)
    {
        if (tokens.empty()) {
            return true;
        }
    ...
    [[nodiscard]] bool should_emit_node(const ExpandedNode& node, const StaticAddressQueryOptions& options)
    {
    ...
    [[nodiscard]] bool should_expand_query_children(const ExpandedNode& node, const bool flatten_composite_members)
    {
                               ...
                               bool flatten_composite_members)
    {
        ...
        for (const auto& child : children) {
            flatten_expanded_node(expander, child, output, visited, max_array_elements, flatten_composite_members);
        }
    }
...
StaticAddressQuerySession::StaticAddressQuerySession(const ProjectModel& model)
    : model_(&model), expanded_nodes_(model.expanded), impl_(std::make_unique<Impl>())
{
}
...
StaticAddressQuerySession& StaticAddressQuerySession::operator=(StaticAddressQuerySession&&) noexcept = default;

std::vector<StaticAddressResult> StaticAddressQuerySession::query(const StaticAddressQueryOptions& options)
{
    if (model_ == nullptr) {
        return {};
    }
    ...
    for (const auto& node : expanded_nodes_) {
        flatten_expanded_node(
            expander, node, flattened_nodes, visited, options.max_array_elements, options.flatten_composite_members);
    }
...
std::vector<StaticAddressResult> query_static_addresses(const ProjectModel& model,
...
std::vector<StaticAddressResult> query_static_addresses_from_file(const std::string& elf_path,
                                                                  const StaticAddressQueryOptions& options,
