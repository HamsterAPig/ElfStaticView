...
namespace {
    ...
    [[nodiscard]] std::string dwarf_name_or_hex(const unsigned int value, int (*resolver)(unsigned int, const char**))
    {
                                                   ...
                                                   const Dwarf_Half form)
    {
        ...
        if (form_class == DW_FORM_CLASS_CONSTANT || form_class == DW_FORM_CLASS_LINEPTR ||
            form_class == DW_FORM_CLASS_LOCLISTPTR || form_class == DW_FORM_CLASS_MACPTR ||
            form_class == DW_FORM_CLASS_RANGELISTPTR || form_class == DW_FORM_CLASS_FRAMEPTR ||
            form_class == DW_FORM_CLASS_MACROPTR || form_class == DW_FORM_CLASS_ADDRPTR ||
            form_class == DW_FORM_CLASS_LOCLIST || form_class == DW_FORM_CLASS_LOCLISTSPTR ||
            form_class == DW_FORM_CLASS_RNGLIST || form_class == DW_FORM_CLASS_RNGLISTSPTR ||
                                                                 ...
                                                                 const Dwarf_Off die_offset_value)
    {
        ...
        if (result != DW_DLV_OK) {
            record_local_error(document, cu_index, die_offset_value, dwarf_error_text("dwarf_attrlist failed", error));
            return attributes;
        }
                                            ...
                                            const std::size_t cu_index)
    {
        ...
        raw_die.attributes = read_attributes(debug, die, version, offset_size, document, cu_index, raw_die.offset);

        try {
            auto child = child_of(debug, die);
            while (child.has_value()) {
                raw_die.children.push_back(
                    read_die_tree(debug, child->get(), is_info, version, offset_size, document, cu_index));
...
RawDwarfDocument RawDwarfReader::load(const std::string& file_path) const
{
    ...
    while (true) {
        ...
        try {
            ...
            compile_unit.root =
                read_die_tree(debug.get(), cu_die.get(), true, version_stamp, length_size, document, index);
