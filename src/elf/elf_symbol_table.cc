...
namespace {
    ...
    template <typename T> std::array<unsigned char, sizeof(T)> read_bytes(std::ifstream& input)
    {
    ...
    template <typename T> T read_integer(std::ifstream& input, const ByteOrder byte_order)
    {
    ...
    template <typename Header> Header read_header(std::ifstream& input, const ElfIdent& ident, ByteOrder byte_order);
    ...
    template <typename Section> Section read_section_header(std::ifstream& input, ByteOrder byte_order);

    template <>
    Elf32SectionHeader read_section_header<Elf32SectionHeader>(std::ifstream& input, const ByteOrder byte_order)
    {
    ...
    template <>
    Elf64SectionHeader read_section_header<Elf64SectionHeader>(std::ifstream& input, const ByteOrder byte_order)
    {
    ...
    template <typename Symbol> Symbol read_symbol(std::ifstream& input, ByteOrder byte_order);

    template <> Elf32Symbol read_symbol<Elf32Symbol>(std::ifstream& input, const ByteOrder byte_order)
    {
        Elf32Symbol symbol;
    ...
    template <> Elf64Symbol read_symbol<Elf64Symbol>(std::ifstream& input, const ByteOrder byte_order)
    {
        Elf64Symbol symbol;
        symbol.name = read_integer<std::uint32_t>(input, byte_order);
        symbol.info = read_integer<unsigned char>(input, byte_order);
...
ElfSymbolTable ElfSymbolTable::load(const std::string& file_path)
{
    ...
    if (elf_class == kElfClass32) {
        ...
        table.symbols_ = load_symbol_table<Elf32Header, Elf32SectionHeader, Elf32Symbol>(input, header, byte_order);
        return table;
    }

    const auto header = read_header<Elf64Header>(input, ident, byte_order);
    table.metadata_ = build_metadata(header, elf_class, byte_order);
    table.symbols_ = load_symbol_table<Elf64Header, Elf64SectionHeader, Elf64Symbol>(input, header, byte_order);
    return table;
}
...
std::optional<std::vector<std::uint8_t>> ElfSymbolTable::read_section_bytes(const std::string& file_path,
                                                                            const std::string& section_name)
{
    std::ifstream input(platform::utf8_path(file_path), std::ios::binary);
    if (!input.is_open()) {
    ...
    if (elf_class == kElfClass32) {
        const auto header = read_header<Elf32Header>(input, ident, byte_order);
        return load_section_bytes<Elf32Header, Elf32SectionHeader>(input, header, byte_order, section_name);
    }

    const auto header = read_header<Elf64Header>(input, ident, byte_order);
    return load_section_bytes<Elf64Header, Elf64SectionHeader>(input, header, byte_order, section_name);
}
