...
namespace {
    ...
    [[nodiscard]] std::vector<std::uint8_t> read_file_header_bytes(const std::string& file_path, const std::size_t size)
    {
        std::ifstream stream(platform::utf8_path(file_path), std::ios::binary);
        if (!stream) {
            throw DwarfError("无法打开对象文件: " + file_path);
    ...
    [[nodiscard]] std::string read_short_name(const std::vector<std::uint8_t>& data, const std::size_t offset)
    {
        std::size_t length = 0;
                                                              ...
                                                              const std::uint32_t symbol_count)
    {
        const auto table_offset =
            static_cast<std::uint64_t>(symbol_offset) + static_cast<std::uint64_t>(symbol_count) * kSymbolRecordSize;
    ...
    {
        return std::any_of(
            sections.begin(), sections.end(), [name](const TiCoffSection& section) { return section.name == name; });
    }
    ...
    [[nodiscard]] Dwarf_Small get_byte_order(void*)
    {
        return DW_END_little;
    }

    [[nodiscard]] Dwarf_Small get_length_size(void*)
    {
        return 4;
    }

    [[nodiscard]] Dwarf_Small get_pointer_size(void*)
    {
        return 4;
    }
    ...
    [[nodiscard]] int relocate_section(void*, Dwarf_Unsigned, Dwarf_Debug, int*)
    {
        return DW_DLV_OK;
    }
...
TiCoffObject::TiCoffObject(std::string file_path) : file_path_(std::move(file_path))
{
    ...
    const auto section_table_size = static_cast<std::uint64_t>(section_count) * kSectionHeaderSize;
...
const std::string& TiCoffObject::file_path() const noexcept
{
    return file_path_;
}

Dwarf_Unsigned TiCoffObject::file_size() const noexcept
{
    return file_size_;
}

Dwarf_Unsigned TiCoffObject::section_count() const noexcept
{
...
std::vector<std::uint8_t> TiCoffObject::read_section_data(const Dwarf_Unsigned section_index) const
{
    if (section_index == 0U) {
...
ObjectFileKind detect_object_file_kind(const std::string& file_path)
{
    ...
    if (data.size() >= 2) {
        const auto magic = static_cast<std::uint16_t>(data[0]) | (static_cast<std::uint16_t>(data[1]) << 8U);
