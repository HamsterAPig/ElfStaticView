...
int run_with_access_violation_guard(EntryRunner runner, const int argc, char** argv)
{
    __try {
        ...
        elf_static_view::logging::log(elf_static_view::logging::Level::Error,
...
int run_cli(const int argc, char** argv)
{
    ...
    const bool include_runtime_only =
        options.show_runtime_only_specified ? options.show_runtime_only : !load_policy.exclude_runtime_only_variables;
    const bool only_static_known =
        options.only_static_known_specified ? options.only_static_known : load_policy.static_storage_only;
    const std::size_t expand_depth = options.expand_depth_specified ? options.expand_depth : load_policy.expand_depth;

    if (options.command == "scan") {
        const auto model =
            loader.scan(options.file, {.include_runtime_only = include_runtime_only, .load_policy = load_policy});
...
int run_ui(const int argc, char** argv)
{
    ...
    elf_static_view::ui::Application app({.startup_file = input_path,
