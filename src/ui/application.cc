...
namespace {
    ...
    [[nodiscard]] std::optional<ExpandedNode> filter_export_node(const ExpandedNode& node,
    ...
    [[nodiscard]] std::vector<ExpandedNode> collect_filtered_roots(const AppState& state)
    {
        ...
        analysis::Expander expander(state.project_model->types, state.load_policy.expand_depth, false);
    ...
    [[nodiscard]] ExportDocument build_export_document_from_state(const AppState& state, const ExportOptions& options)
    {
        ...
        if (options.source == ExportSource::CurrentFilteredView) {
            ...
            return ExportDocument{ExportPayloadKind::VariableSummary, build_lightweight_export(nodes, options)};
        }
        return ExportDocument{ExportPayloadKind::VariableSummary, build_lightweight_export(value.model, options)};
    }
    ...
    void configure_ui_font(ImGuiIO& io, const float ui_scale)
    {
        ...
        if (length > 0) {
            ...
            const std::filesystem::path fonts_directory = std::filesystem::path(windows_directory) / "Fonts";
            ...
            for (const wchar_t* font_name : kCandidateFonts) {
                ...
                if (ImFont* font = io.Fonts->AddFontFromFileTTF(utf8_font_path.c_str(),
...
void Application::start_snapshot_import(const std::string& path)
{
    const std::uint64_t task_id =
        task_runner_.submit(kTaskSnapshotImport, path, [path]() -> std::pair<bool, std::string> {
            try {
...
void Application::start_snapshot_export(const std::string& path, const ExportOptions& options)
{
    ...
    const std::uint64_t task_id = task_runner_.submit(
        kTaskSnapshotExport, path, [path, document = std::move(document), options]() -> std::pair<bool, std::string> {
            try {
...
{
    const std::uint64_t task_id = task_runner_.submit(
        kTaskRawDwarfExport, output_path, [source_path, output_path]() -> std::pair<bool, std::string> {
            try {
                ProjectLoader loader;
            ...
            try {
                ...
                if (snapshot.has_value()) {
                    return std::pair<bool, std::string>{
                        true,
                        render_snapshot_json(snapshot.value(), {.include_sensitive_info = include_sensitive_info})};
                }
...
void Application::start_version_check()
{
    ...
    const std::uint64_t task_id = task_runner_.submit(
        kTaskVersionCheck, effective_state.check_uri, [effective_state]() -> std::pair<bool, std::string> {
            try {
    ...
    state_.filters.tasks.push_back(FilterTask{
        ...
        .future = std::async(std::launch::async,
                             [model_snapshot, rules, expand_depth, lazy_expand_children]() {
                                 return build_filter_cache(
                                     model_snapshot.get(), rules, expand_depth, lazy_expand_children);
                             }),
    });
    request_redraw();
}
...
void Application::poll_background_tasks()
{
    ...
    for (auto iter = state_.filters.tasks.begin(); iter != state_.filters.tasks.end();) {
        if (!iter->future.valid() || iter->future.wait_for(std::chrono::milliseconds(0)) != std::future_status::ready) {
            ++iter;
            continue;
        }
    ...
    if (const auto exported = task_runner_.poll(kTaskRawDwarfExport); exported.has_value()) {
        if (exported->success) {
            finish_ui_task(
                state_.export_raw_dwarf_task, exported->id, "已导出原始 DWARF JSON: " + exported->success_value);
    ...
    if (const auto version = task_runner_.poll(kTaskVersionCheck); version.has_value()) {
        if (version->success) {
            try {
                const VersionCheckState effective_state = resolve_version_check_state(state_);
                state_.version_check = parse_version_response_text(
                    version->success_value, effective_state.check_uri, effective_state.repository_url);
