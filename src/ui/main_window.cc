...
namespace {
    ...
    void handle_global_shortcuts(AppState& state)
    {
        ...
        if (!io.WantTextInput && ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_F, ImGuiInputFlags_RouteGlobal)) {
            state.focus_variable_search = true;
        }
        if (!io.WantTextInput && ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, ImGuiInputFlags_RouteGlobal)) {
            copy_selected_adjusted_address(state);
        }
        if (!io.WantTextInput && ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_B, ImGuiInputFlags_RouteGlobal)) {
    ...
    void render_tree_node(AppState& state, const ExpandedNode& node)
    {
        ...
        const auto flags = ImGuiTreeNodeFlags_SpanAvailWidth | (!has_children ? ImGuiTreeNodeFlags_Leaf : 0) |
                           ((state.selected_node_path == node.path) ? ImGuiTreeNodeFlags_Selected : 0);

        const auto label = node.display_name + " [" + node.type_name + "] @ " +
        ...
        if (ImGui::BeginPopupContextItem()) {
            ...
            if (const auto copied_text = format_adjusted_address_for_copy(node, state); copied_text.has_value()) {
        ...
        if (opened) {
            ...
            if (node.children_lazy) {
                analysis::Expander expander(
                    state.project_model->types, state.load_policy.expand_depth, state.load_policy.lazy_expand_children);
    ...
    void render_filters(AppState& state)
    {
        ...
        if (ImGui::InputTextWithHint(
                kVariableSearchInputId, "匹配变量名或完整路径中的子串", &state.filters.form.variable_name_query)) {
        ...
        if (ImGui::Checkbox("将地址偏移写回配置文件", &state.persist_address_bias_to_config)) {
            ...
            log_info(
                state,
                state.persist_address_bias_to_config ? "已启用地址偏移写回配置文件" : "已禁用地址偏移写回配置文件");
        }
    ...
    void render_variables_panel(AppState& state)
    {
        ...
        if (state.background_load.status == BackgroundLoadStatus::Loading) {
            ...
            const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() -
                                                                                  state.background_load.started_at);
    ...
    void render_inspector_panel(AppState& state)
    {
        ...
        if (state.project_model) {
            ...
            if (state.load_policy.enable_parse_metrics) {
                ...
                ImGui::Text("过滤前变量: %llu", static_cast<unsigned long long>(metrics.variable_count_before_filter));
                ImGui::Text("过滤后变量: %llu", static_cast<unsigned long long>(metrics.variable_count_after_filter));
                ImGui::Text("跳过 CU: %llu", static_cast<unsigned long long>(metrics.skipped_compile_unit_count));
            }
            ImGui::Separator();
        }
        const auto* selected_node = resolve_selected_node(state);
        ...
        ImGui::Text("偏移后地址: %s", elf_static_view::format_address_summary(node, state.address_bias).c_str());
    ...
    void render_log_panel(AppState& state)
    {
        ...
        ImGui::InputTextMultiline(
            "##log_panel_text", &log_text, ImVec2(-FLT_MIN, -FLT_MIN), ImGuiInputTextFlags_ReadOnly);
        ImGui::End();
    }
    ...
    void render_about_dialog(AppState& state)
    {
        ...
        if (ImGui::BeginPopupModal(kAboutDialogName, &state.show_about_dialog, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            ImGui::TextWrapped("更新来源: %s",
                               effective_version_check.check_uri_uses_default ? "默认 GitHub Releases API"
    ...
    void render_shortcuts_dialog(AppState& state)
    {
        ...
        if (ImGui::BeginPopupModal(
                kShortcutsDialogName, &state.show_shortcuts_dialog, ImGuiWindowFlags_AlwaysAutoResize)) {
...
bool MainWindow::render(AppState& state)
{
    ...
    const bool preview_related_changed =
        state.selected_node_path != previous_selected_path || state.show_json_preview_panel != previous_show_preview ||
        state.export_sensitive_info != previous_export_sensitive || state.current_file_path != previous_file_path ||
        state.current_snapshot_path != previous_snapshot_path || state.loaded_kind != previous_loaded_kind;
