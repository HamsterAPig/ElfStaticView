...
namespace {
    ...
    std::string abbreviate_directory_for_title(const std::filesystem::path& directory_path)
    {
        ...
        std::size_t cursor = 0;
        if (generic_path.size() >= 2 && std::isalpha(static_cast<unsigned char>(generic_path[0])) != 0 &&
            generic_path[1] == ':') {
...
std::optional<std::string> format_adjusted_address_for_copy(const ExpandedNode& node, const AppState& state)
{
                        ...
                        const std::string& source_path)
{
    ...
    state.filters.last_input_at =
        std::chrono::steady_clock::now() - std::chrono::milliseconds(state.filter_debounce_ms);
...
{
    return !state.opened_file_monitor.path.empty() && state.opened_file_monitor.content_kind != LoadedContentKind::None;
}
