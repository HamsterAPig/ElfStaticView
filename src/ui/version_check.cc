...
namespace {
    ...
    constexpr char kDefaultReleasesApiUrl[] = "https://api.github.com/repos/HamsterAPig/ElfStaticView/releases/latest";
    ...
    std::filesystem::path config_path_for_executable(const std::filesystem::path& executable_path)
    {
        const auto parent_path =
            executable_path.has_parent_path() ? executable_path.parent_path() : std::filesystem::current_path();
    ...
    std::string http_get_text_impl(const std::string& uri_text)
    {
        ...
        const HINTERNET session = WinHttpOpen(
            L"ElfStaticView/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
        if (session == nullptr) {
            throw std::runtime_error("版本检查无法初始化 HTTP 会话");
        }

        const HINTERNET connection = WinHttpConnect(session, uri.host.c_str(), uri.port, 0);
        ...
        const HINTERNET request = WinHttpOpenRequest(
            connection, L"GET", uri.path.c_str(), nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
        if (request == nullptr) {
        ...
        if (WinHttpSendRequest(request, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0) == FALSE ||
                                              ...
                                              const std::string& repository_url)
{
    ...
    result.release_notes = first_non_empty(read_yaml_scalar(root, "body"), read_yaml_scalar(root, "notes"));
    ...
    if (result.has_new_version) {
        message << "发现新版本 " << result.latest_version << "，当前版本 " << current_version_string();
    } else {
...
{
    LoadPolicy policy;
    policy.compile_unit_path_rules_text = "# 默认排除常见通用库目录\n"
                                          "**/CMSIS/**\n"
                                          "**/HAL/**\n"
                                          "**/Drivers/**\n"
                                          "**/Middlewares/**\n"
...
void load_app_config(AppState& state, const std::filesystem::path& executable_path)
{
    ...
    state.enable_background_loading = read_yaml_bool(load_policy, "enable_background_loading", true);
    state.load_policy.static_storage_only = read_yaml_bool(load_policy, "default_static_storage_only", true);
    state.load_policy.exclude_formal_parameters = read_yaml_bool(load_policy, "exclude_formal_parameters", true);
    state.load_policy.exclude_runtime_only_variables =
        read_yaml_bool(load_policy, "exclude_runtime_only_variables", true);
    state.load_policy.lazy_expand_children = read_yaml_bool(load_policy, "lazy_expand_children", true);
    state.load_policy.enable_parse_metrics = read_yaml_bool(load_policy, "enable_parse_metrics", true);
    ...
    if (const std::string rules = read_yaml_scalar(load_policy, "compile_unit_path_rules"); !rules.empty()) {
        if (const auto validation_error = validate_compile_unit_path_rules_text(rules); validation_error.has_value()) {
            log_error(state,
...
void save_app_config(const AppState& state)
{
    ...
    root["load_policy"]["exclude_runtime_only_variables"] = state.load_policy.exclude_runtime_only_variables;
    ...
    root["export"]["lightweight_max_array_elements"] = state.export_dialog.options.lightweight_max_array_elements;
