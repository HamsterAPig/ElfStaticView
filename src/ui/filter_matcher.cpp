...
namespace {
    ...
    {
        return left.variable_name_query == right.variable_name_query && left.path_rules_text == right.path_rules_text &&
    ...
    void collect_visible_paths(AppState& state, const ExpandedNode& node, const analysis::Expander* expander)
    {
...
bool receive_filter_build_result(AppState& state, const std::uint64_t task_id, FilterBuildResult result)
{
...
void rebuild_filter_cache(AppState& state)
{
    ...
    if (cache.valid && cache.model == model && cache.expand_depth == state.load_policy.expand_depth &&
    ...
    analysis::Expander expander(model->types, state.load_policy.expand_depth, state.load_policy.lazy_expand_children);
    for (const auto& node : model->expanded) {
