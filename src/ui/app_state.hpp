...
struct SharedProjectModel {
    ...
    [[nodiscard]] bool has_value() const noexcept { return storage != nullptr; }

    explicit operator bool() const noexcept { return has_value(); }

    [[nodiscard]] ProjectModel* get() noexcept { return storage.get(); }

    [[nodiscard]] const ProjectModel* get() const noexcept { return storage.get(); }
    ...
    [[nodiscard]] ProjectModel& operator*() { return value(); }

    [[nodiscard]] const ProjectModel& operator*() const { return value(); }

    [[nodiscard]] ProjectModel* operator->() noexcept { return storage.get(); }

    [[nodiscard]] const ProjectModel* operator->() const noexcept { return storage.get(); }
    ...
    [[nodiscard]] std::shared_ptr<const ProjectModel> share_readonly() const noexcept { return storage; }
};
...
void set_loaded_project(AppState& state, ProjectModel model, LoadedContentKind kind, const std::string& source_path);
...
[[nodiscard]] bool opened_file_monitor_check_due(AppState& state, std::chrono::steady_clock::time_point now);
[[nodiscard]] std::optional<std::string> observe_opened_file_presence(AppState& state,
                                                                      bool exists,
